...
- **今日养猫状态看板**：4 张数据卡片，点击直达对应工具
  - 疫苗倒计时（3 天内到期/逾期红色高亮） · 今日喂食次数 · 日记更新距今天数 · 我的猫咪档案
- **危险食物指南横幅**：看板下方红色警示入口，直达危险食物指南
- **养猫小知识**：随机冷知识 / 小贴士卡片
- **搜索入口**：跳转到图鉴搜索
...
| ⚖️ 体重追踪 | P1 | 按猫记录体重，趋势图 + 品种参考区间评估 |
| 🧮 养猫成本计算器 | P2 | 初期投入 + 月均 / 首年 / 日均成本测算，含消费水平预设 |

品种真实图片仍在规划中。品种详情、日记、首页和成本计算器已支持分享给好友与朋友圈；日记支持添加本地配图。工具页底部提供意见反馈入口（反馈本地留存 + 一键复制 + 微信官方反馈渠道）与数据管理页（备份导出为文件 / 复制文本，导入按模块覆盖恢复，直接复用 persist 信封与版本迁移）。

### 模块四：猫咪档案（多猫管理）
- **建立档案**：名字、生日、性别、品种（关联图鉴）、绝育状态、emoji 头像 + 配色
- **档案卡片自动汇总**：年龄 / 人龄换算 / 生命阶段、疫苗倒计时（临近高亮）、喂食与日记记录数、最新体重，点击数据格直达对应工具
- **一键选猫**：日记 / 喂食 / 疫苗表单的"猫咪名字"升级为档案快捷选择（共用 `CatNamePicker` 组件），数据仍以 `catName` 字符串关联，零迁移成本
